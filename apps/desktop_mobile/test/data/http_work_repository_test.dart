...
void main() {
  group('HttpWorkRepository decoding', () {
    test(
      'decodes API JSON into domain models with runId on artifacts',
      () async {
    ...
    test(
      'side-endpoint failure inside getWorkItem yields empty list (not crash)',
      () async {
        final api = _FakeControlApi(
          detail: {
            'id': 'wi_3',
    ...
    test(
      'returns null (not fixture leak) when getWorkItem returns empty body',
      () async {
        final api = _FakeControlApi(detail: const {});
        final item = await HttpWorkRepository(
          api: api,
        ).getWorkItem('does-not-exist');
        expect(
          item,
          isNull,
          reason: 'Empty API body must NOT silently fall back to fixture data.',
        );
      },
    );

    test(
      'unknown surface validation defaults to UNVALIDATED, not validated',
      () async {
        ...
        expect(
          surfaces.single.validation,
          WorkItemSurfaceValidation.unvalidated,
        );
      },
    );
  });
