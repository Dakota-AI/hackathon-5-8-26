...
> Single-page checklist of every meaningful component. Linked to its detailed page. Updated 2026-05-10 after `b515e14` (web redesign), `b4d18fc` (Flutter auth gate), `d8c2a22` (real Hermes), `febccc1` (resident ECS runtime), `ba54101` (surface validation).
...
**Stateless smoke worker (SFN-driven):**
...
- [ ] **Real model invocation in stateless worker** — `HERMES_RUNNER_MODE=smoke` and image has no `hermes` binary ❌
- [ ] Worker seq numbers hardcoded — retry-fragile ⚠️

**Resident runner:**
- [x] Dockerfile.resident multi-stage build ✅
- [x] **Real Hermes baked in** (Stage 2 = `nousresearch/hermes-agent:latest` with `/opt/hermes/.venv/bin/hermes`) ✅
- [x] `runAdapter` defaults to `hermes-cli` (smoke adapter REMOVED in `d8c2a22`) ✅
- [x] `resident-entrypoint.sh` materializes `HERMES_AUTH_JSON_BOOTSTRAP` to `$HERMES_HOME/auth.json` ✅
- [x] Bearer-token-gated HTTP server on :8787 with `/health /state /events /agents /credentials/hermes-auth /wake /shutdown` ✅
...
- [ ] Real tool execution beyond hermes-cli (file ops, shell, web, MCP) ❌
- [ ] Sandbox / cgroup / network egress policy ❌
- [ ] Provider quota (Codex) ⚠️
...
After commit `b515e14` redesign:
- [x] Amplify Auth (Authenticator modal + SignInModal + dev bypass) ✅
- [x] WorkspaceProvider (`useWorkspace()`, persisted to localStorage; switcher in topbar) ✅
- [x] `POST /runs` create run, polling `/events`, full work-item lifecycle ✅
- [x] `/runs` Open-WebUI-style chat (work-item-keyed conversations) ✅
- [x] `/artifacts` board with presigned download URLs ✅
- [x] `/approvals` board with Approve/Deny POST decision ✅
- [x] GenUI renderer with 19-component allowlist + markdown sanitization ✅
...
- [x] Helper functions for /work-items, /runs, /artifacts/download, /approvals/decision, /agent-profiles, /admin/* ✅
- [ ] WebSocket realtime (helpers exist, not consumed — polls instead) ⚠️
- [ ] `MetricsStrip`, `LiveRunTimeline`, `GenUiPreviewPanel` on `/` still static ❌
- [ ] `/agents` and `/miro` are placeholder pages ❌
- [ ] No `/workspaces` discovery API (workspace IDs seeded client-side) ⚠️
- [ ] No reason input on Approvals decision; "Request revision" button disabled ⚠️
- [ ] No component-level / Playwright tests ⚠️

### Flutter (desktop_mobile) — see [flutter.md](clients/flutter.md)

After commit `b4d18fc` (auth + transport real, render paths still fixture):
- [x] Amplify config loaded at boot ✅
- [x] **`_AuthGate` with real Cognito sign-in/sign-up/confirm** via `auth_controller.dart` ✅
- [x] **`SignInPage` with shadcn-style tabs** + "Continue without sign-in (local fixtures)" ghost button ✅
- [x] **`controlApiProvider`** — typed `ControlApi` with 8 endpoints + Bearer ID-token ✅
- [x] **`realtimeClientProvider`** — real `web_socket_channel` wss client with `subscribeRun` ✅
- [x] Sidebar redesign: Agents / Kanban / Approvals / Browser / GenUI Lab / UI Kit ✅
- [x] Agents workspace + AgentDetailPage with Overview / Activity / Artifacts / Approvals tabs ✅
- [x] Kanban board (Todo / In Progress / Review / Done) ✅
- [x] Real `webview_flutter` browser (https-only, JS off by default) ✅
- [x] `_GenUiLabPage` with real `genui.SurfaceController` + `fl_chart` charts ✅
- [x] SquaresLoader ✅
- [x] Tests reworked, 5/5 green ✅
- [ ] **Page bodies still consume `FixtureWorkRepository`** — `controlApiProvider` and `realtimeClientProvider` declared but never read by render widgets ⚠️
- [ ] No live data on Agents workspace, Kanban, Approvals, Artifacts ⚠️
- [ ] `_GenUiLabPage` GenUI surface uses local seed only, not API patches ⚠️
- [ ] Hardcoded Cognito IDs in `backend_config.dart:8-13` ⚠️
- [ ] `go_router` declared but unused ⚠️
...
- [x] Owner-scoped reads on `getRun`, `requireOwnedWorkItem`, `requireOwnedProfile`, `requireOwnedApproval`, `requireOwnedArtifact`, `requireOwnedSurface`, `requireOwnedDataSourceRef` ✅
